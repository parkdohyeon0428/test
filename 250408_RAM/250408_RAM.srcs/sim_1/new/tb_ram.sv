...
interface ram_intf(
    ...
    // 타이밍을 맞춰주는 기능 , test bench 기준으로 봐야함.
    clocking cb @(posedge clk);// clk을 기준으로 신호값의 딜레이를 시켜줌 
...
class generator;
    ...
    task run (int repeat_counter);
        transaction ram_tr;
        repeat (repeat_counter)begin // 반복
            ram_tr = new();
